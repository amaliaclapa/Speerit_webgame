...
// === SCRIPT REORGANIZAT ȘI FUNCȚIONAL ===

const players = []; // jucători la nivel global
...
// ✅ Acum showDice e vizibil global
...
document.addEventListener("DOMContentLoaded", () => {
    showDice(1); // afișează un zar inițial
});
...
function moveCurrentPlayer(diceValue) {
    ...
    // 🔰 Dacă nu e pe tablă, plasează-l pe poziția 1 și ajustează
    if (current === 0) {
...
document.addEventListener('DOMContentLoaded', () => {
 ...
 function showAnswerPanel(question, color) {
    if (isAnswerPanelOpen) return; // ⛔ prevenim deschiderea mai multor panouri

    isAnswerPanelOpen = true;

    const panel = document.createElement('div');
    panel.classList.add('answer-panel', color); // .answer-panel.yellow sau .purple
    ...
    closePanel(question, answer, color); // ✅ trimitem întrebarea și culoarea
});

function closePanel(questionText, answer, color) {
    if (panel && panel.parentNode) {
        panel.remove(); // elimină în siguranță
    }

    isAnswerPanelOpen = false;

    // 👉 după ce s-a închis answerPanel-ul, afișăm panelul pentru echipă
...
function showTeamResponsePanel(question, playerAnswer, color) {
    ...
    const teamPanel = document.createElement('div'); // ✅ creezi întâi
    teamPanel.classList.add('team-response-panel');

    // ✅ apoi aplici culoarea
    if (color === "yellow") {
